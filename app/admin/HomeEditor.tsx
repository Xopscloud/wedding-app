        ...
        <div className="mt-2 flex gap-2">
          <button disabled={!file || uploading} onClick={async ()=>{
            ...
            try{
              ...
              form.append('albumKey', albumKey)
              const res = await fetch(`${API_BASE}/api/admin/album-cover`, { method: 'POST', headers: { 'x-admin-password': pass }, body: form })
              if(!res.ok){ const t = await res.text(); throw new Error(t || 'Upload failed') }
              const data = await res.json()
              const uploadedUrl = data.image
